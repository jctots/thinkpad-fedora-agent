...
- [ ] Stand up the `/var/home` backup — kopia to the home-lab NAS over Tailscale (§3.4b). [`recovery.md`](recovery.md) Card 3 has the install and connect commands, verified end-to-end on 2026-08-19 (see `incidents/I013`); the connect step needs `--sftp-password`/`--password` and a `--known-hosts` file with *every* algorithm the host offers (`ssh-keyscan -p 22 <host>`, not just the one your last `ssh` session negotiated) or it fails with `knownhosts: key mismatch`. **Run a snapshot once, then pull one file back out.** Nothing else covers `/var/home`, and an untested backup is not a layer — it is a belief
...
- [ ] `scripts/system-health-check.sh install` — enables the 15-min timer that watches pending updates plus this host's known failure modes (fprintd crash-loop, tb-bridge crash-loop, battery drain) and notifies on genuine failure only. Without this step the checks exist in the repo but nothing runs them; see `incidents/I029`/`I030`
