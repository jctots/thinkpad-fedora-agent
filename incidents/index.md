...
|---|---|---|---|---|---|---|
| [I011](I011-sudo-gui-auth-for-agent-pkexec-over-askpass.md) | 2026-08-17 | agent | Agent-run `sudo` needs GUI auth (no controlling TTY); `openssh-askpass` worked but fingerprint prompt never surfaced — `pkexec` (existing polkit agent) supports both, no extra package | ~45m | ✓ | not yet — one-off, use `pkexec <cmd>` going forward |
