...
| [I008](I008-build-signed-kmod-toolbox-key-access.md) | 2026-08-17 | toolbox | `build-signed-kmod.sh`'s first live run fails to read the MOK private key — wrong path inside toolbox, and even the right path is unreadable due to rootless-container permissions. **Resolved 2026-08-25**: split the privileged read to a new host-side `stage-mok-key.sh`; four more untested bugs in the script fixed along the way; verified end to end for nvidia and xpadneo | unresolved 2026-08-17, ~40m to resolve 2026-08-25 | ✗ both sessions | `scripts/build-signed-kmod.sh`, `scripts/stage-mok-key.sh` |
